...
Shader "Custom/RippleShaderUnlit" {
	Properties{
		...
		_OriginX("Origin X", Float) = 0.0
		_OriginY("Origin Y", Float) = 0.0
		_OriginZ("Origin Z", Float) = 0.0
	}
	SubShader{
		...
		void vert(inout appdata_full v, out Input o)
		{
			...
			float dist = distance(fixed3(_OriginX, _OriginY, _OriginZ), worldPos);
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			float dist = distance(fixed3(_OriginX, _OriginY, _OriginZ), IN.worldPos);
			float val = abs(sin(dist*_RippleInterval - _Time * _RippleSpeed));
